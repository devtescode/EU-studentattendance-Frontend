...
import { LayoutDashboard, Users, GraduationCap, BookOpen, ClipboardList, FileBarChart, CalendarClock, LogOut } from "lucide-react";
        ...
        <Sidebar collapsible="icon">
          <SidebarHeader className="border-b border-sidebar-border">
            <div className="flex items-center gap-2 px-2 py-3">
              <div className="h-9 w-9 shrink-0 rounded-lg bg-[#006B3C] grid place-items-center text-white font-bold">E</div>
              <div className="min-w-0">
                <p className="text-sm font-semibold truncate">Elizade University</p>
                <p className="text-xs text-muted-foreground truncate">Attendance System</p>
              </div>
            </div>
          </SidebarHeader>
          <SidebarContent>
...
export function PageHeader({ title, subtitle, actions }: { title: string; subtitle?: string; actions?: React.ReactNode }) {
  return (
    <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
      <div className="min-w-0">
        <h2 className="text-2xl sm:text-3xl font-bold text-[#1C1C1C]">{title}</h2>
...
export function StatCard({ label, value, tone = "green" }: { label: string; value: React.ReactNode; tone?: "green" | "gold" | "blue" | "slate" }) {
  ...
  return (
    <div className="rounded-xl bg-white border p-5 shadow-sm">
      ...
      <div className="mt-2 flex items-end justify-between">
        ...
        <span className={`h-9 w-9 rounded-lg grid place-items-center text-sm font-bold ${tones[tone]}`}>★</span>
      </div>
    </div>
  );
}